...
C+-----------------------------------------------------------------------+
C      CALL DIRheader(logfile, version, x, n, eps, maxf, maxT, l, u,
C     +               algmethod, maxfunc, maxdeep, fglobal, fglper,
C     +               Ierror, epsfix, iepschange, volper, sigmaper,
C     +               iidata, iisize, ddata, idsize, cdata,
C     +               icsize)
C+-----------------------------------------------------------------------+
C| If an error has occured while writing the header (we do some checking |
C| of variables there), return to the main program.                      |
C+-----------------------------------------------------------------------+
C      IF (Ierror .lt. 0) then
C         RETURN
C      END IF
C+-----------------------------------------------------------------------+
...
C        write(*,10012) tstart-1,numfunc
C        write(logfile,10012) t,numfunc
      ELSE
C        Write(*,10002) numfunc, fmin, fmax
C        Write(logfile,10003) tstart-1,numfunc,fmin
      END IF
C+-----------------------------------------------------------------------+
C+-----------------------------------------------------------------------+
C| Main loop!                                                            |
...
C          Write(*,10002) numfunc,fmin, fmax
C          Write(logfile,10003) t,numfunc,fmin
        END IF
C+-----------------------------------------------------------------------+
C| If no feasible point has been found, give out the iteration, the      |
...
C+-----------------------------------------------------------------------+
        ...
        IF (delta .LE. volper) THEN
           Ierror = 4
C           Write(*,10011) delta, volper
C           Write(logfile,10011) delta, volper
           GOTO 100
        END IF
C+-----------------------------------------------------------------------+
...
C+-----------------------------------------------------------------------+
        ...
        IF (delta .LE. sigmaper) THEN
           Ierror = 5
C           Write(*,10013) delta, sigmaper
C           Write(logfile,10013) delta, sigmaper
           GOTO 100
        END IF
C+-----------------------------------------------------------------------+
...
C+-----------------------------------------------------------------------+
        IF (numfunc .GT. maxf) THEN
           IF (Ifeasiblef .eq. 0) then
              Ierror = 1
C              Write(*,10008)
C              Write(logfile,10008)
              GOTO 100
           ELSE
              increase = 1
C              write(logfile,10030) numfunc
              maxf = numfunc+ oldmaxf
           END IF
        END IF
10    CONTINUE
...
C+-----------------------------------------------------------------------+
C      writed = 0
C      IF (writed .EQ. 1) THEN
C        file  = 12
C        file2 =  0
C        CALL DIRWritehistbox(point,f,thirds,c,anchor,actdeep,file,
C     +               l,u,file2,maxfunc,maxdeep,n,MaxDim,length)
C      END IF

C+-----------------------------------------------------------------------+
C| Give out a summary of the run.                                        |
C+-----------------------------------------------------------------------+
C      CALL DIRsummary(logfile,x,l,u,n,fmin,fglobal, numfunc, Ierror)
